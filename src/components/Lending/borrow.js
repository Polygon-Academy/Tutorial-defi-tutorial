  ...
  return (
    <Table basic="very">
        ...
        {Boolean(tokens.length) &&
          _tokens.map((token, i) => (
            <Table.Row key={token.symbol}>
              <Table.Cell
                className="ListTableImage"
                style={{ paddingTop: "1.3rem" }}
              >
                <img src={token.imageUrl} alt=""></img>
                {token.symbol}{" "}
              </Table.Cell>
              ...
              <Table.Cell>
                {" "}
                {token.borrowAPY ? `${token.borrowAPY} %` : `-`}{" "}
              </Table.Cell>
              <Table.Cell>
                <Button
                  basic
                  color="teal"
                  size="small"
                  onClick={(e) => borrowModal(token, userDatas)}
                >
                  Borrow
                </Button>
                ...
                {userDatas[token.symbol].borrowBalance > 0.01 && (
                  <Button
                    ...
                    color="grey"
                    size="small"
                    onClick={(e) => repayModal(token, userDatas)}
                  >
                    Repay
                  </Button>
                )}
      ...
      <BorrowModal
        ...
        UserTokenData={ModalUserTokenData}
      />
      <RepayModal
...
const BorrowModal = (props) => {
  ...
  const handleBorrow = async () => {
    ...
    const tx = await LendingContract.borrow(
      Token.tokenAddr,
      ethers.utils.parseUnits(borrowAmount.toString(), 18)
    );
  ...
  const CalculateBorrow = () => {
    const maxBorrow =
      (UserData.tCollateralBalance - UserData.tBorrowBalance) / Token.price;
    return maxBorrow < Token.tAvaiLiquidity
      ? fixNumber(maxBorrow)
      : fixNumber(Token.tAvaiLiquidity);
  };
      ...
      <Modal.Content>
        <Container>
          ...
          <Item
            ...
            content={`${fixNumber(
              (UserData.tBorrowBalance / UserData.tCollateralBalance) * 100
            )}% -> ${fixNumber(
              ((UserData.tBorrowBalance + borrowAmount * Token.price) /
                UserData.tCollateralBalance) *
                100
            )}% `}
          />
          <Item
            title="Borrow Limit"
            content={`$${UserData.tCollateralBalance} -> $${fixNumber(
              UserData.tCollateralBalance - borrowAmount * Token.price
            )}`}
          />
          <h4>How much would you like to borrow?</h4>
          <p>
            You can set the amount you want to borrow or use the percentage
            buttons below. These percentages are calculated from Borrow limit –
            Borrow limit used. We don't recommend borrowing at 100% to avoid
            liquidation.
          </p>
          <Input
            labelPosition="right"
          ...
          <ButtonPercent
            Liquidity={CalculateBorrow()}
            setUpdate={setBorrowAmount}
          />

          <p>
            You can borrow a maximum of {CalculateBorrow()} {Token.symbol}
          </p>
        </Container>
      </Modal.Content>
      <Modal.Actions>
...
const RepayModal = (props) => {
  ...
  const handleRepay = async () => {
    ...
    await ERC20Contract.approve(
      lendingPoolAddress,
      ethers.utils.parseUnits(repayAmount.toString(), 18)
    );
    const tx = await LendingContract.repayByAmount(
      Token.tokenAddr,
      ethers.utils.parseUnits(repayAmount.toString().slice(0, 8), 18)
    );
  ...
  const CalculateRepay = () => {
    ...
    return maxRepay > UserTokenData.tBorrowBalance
      ? fixNumber(UserTokenData.liquidityBalance)
      : fixNumber(maxRepay);
  };
        ...
        <Container>
          ...
          <Item
            ...
            content={`${fixNumber(
              ...
              ((UserData.tBorrowBalance - repayAmount * Token.price) /
                UserData.tCollateralBalance) *
                100,
              2
            )} `}
          ...
          <p>
            You can set the amount you want to repay or use the percentage
            buttons below. These percentages are according to your 'Wallet
            balance'..
          </p>
          ...
          <ButtonPercent
            Liquidity={CalculateRepay()}
            setUpdate={setRepayAmount}
          />

          <p>
            You can repay a maximum of {CalculateRepay()} {Token.symbol}
          </p>
        </Container>
      </Modal.Content>
      <Modal.Actions>
